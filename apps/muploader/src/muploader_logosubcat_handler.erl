-module(muploader_logosubcat_handler).
...
init(_Type, Req, []) ->
        {ok, Req, undefined}.
handle(Req, State) ->
    lager:debug("LOGOSUBCAT HANDLE: ~p ~p", [Req, State]),
    ...
    lager:debug("LogoSubcat HANDLE AccountID: ~p", [{AccountID,IsAdmin}]),
    
    {Ret, Req2} = multipart(Req1, []),
    lager:debug("logosubcat multipart ret: ~p", [Ret]),
    ...
    lager:debug("logosubcat HANDLE4: ~p", [Req4]),
    {ok, Req4, State}.
...
save_upload(Params) ->
    ...
    case {AccountID, IsAdmin} of
        {Aid, _} when is_integer(Aid), Aid>0 ->
            lager:debug("SAVE subcat logo file :: ~p", [{AccountID, FI,  SCID}]),
            CcR = rpc:call('coleman@127.0.0.1', model_categories, save_logo_file, [FI, SCID]),
...
get_file_name(UploadFileName0, InputName) ->
    ...
    TempDirectory = muploader_utils:get_tmp_dir(logo, InputName),
