...
init(_Type, Req, []) ->
        {ok, Req, undefined}.
handle(Req, State) ->
    lager:debug("MP3 HANDLE: ~p ~p", [Req, State]),

    {MKHCookie, Req1} = cowboy_req:cookie(<<"MIJKSSID">>, Req),
    lager:debug("BCH HANDLE MKHCookie: ~p", [MKHCookie]),
    {AccountID, IsAdmin} = try rpc:call('coleman@127.0.0.1', mijkweb_session, check_session_data, [ellimcd, MKHCookie]) of
            {ok, [_,_,SData]} -> {proplists:get_value(<<"accountid">>, SData, 0), proplists:get_value(<<"is_admin">>, SData, 0)}
            ;_ -> {0,0}
    catch
            _:_ -> {0,0}
    end,
    lager:debug("Mp3 HANDLE AccountID: ~p", [{AccountID,IsAdmin}]),
    
    {Ret, Req2} = multipart(Req1, []),
    lager:debug("MP3 multipart ret: ~p", [Ret]),
    ...
    Mp3 = case proplists:get_value(<<"mkh_mp3_file">>, Ret) of
        <<"/home/ubuntu/work/dev2_coleman/html/media/mp3/", R/binary>> -> R
        ;R -> R
    end,

    {ok, Req4} = reply(jiffy:encode({[{<<"status">>, <<"ok">>}, {<<"file">>,   Mp3} ]}), Req2),   
    lager:debug("MP3 HANDLE4: ~p", [Req4]),
    {ok, Req4, State}.
...
save_upload(Params) ->
    ...
    Name      = proplists:get_value(<<"name">>, Params, 0),
    SCID      = proplists:get_value(<<"sub_cat_id">>, Params, 0),
    lager:debug("AID ~p, IsAdm ~p",[AccountID, IsAdmin]),
    
    case {AccountID, IsAdmin} of
        {Aid, _} when is_integer(Aid), Aid>0 ->
            lager:debug("SAVE mp3 file :: ~p", [{AccountID, FI, Name, SCID}]),
            CcR = rpc:call('coleman@127.0.0.1', model_categories, save_mp3_file, [AccountID, FI, Name, SCID]),
                ...
                {file, FileInpName, Filename, _CType, _CTransferEncoding} ->
                    ...
                    case FileInpName of
                         FIN when FIN=:=<<"mbd_upload">>;FIN=:=<<"file">> ->
                            ...
                            {Req3 ,[{<<"mkh_mp3_file">>, SaveFileName1}]};
                        _ -> 
...
get_file_name(UploadFileName0, InputName) ->
    UploadFileName1 = re:replace(UploadFileName0, "\\s", "", [global, {return, binary}]),
    UploadFileName = case re:replace(UploadFileName1, "[^A-Za-z0-9._-]","", [global, {return, binary}]) of
        <<>>       -> <<"nn.mp3">>;
        <<".mp3">> -> <<"nm.mp3">>;
        UFNOK      -> UFNOK
    end,
    lager:debug("GFN0: ~p ~p", [UploadFileName, InputName]),
    TempDirectory = muploader_utils:get_tmp_dir(mp3, InputName),
