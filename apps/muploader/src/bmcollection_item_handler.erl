-module(bmcollection_item_handler).
...
handle(Req, State) ->
    ...
    Type = case cowboy_req:path(Req) of
 	{<<"/item/bmcollection/create">>, _} -> 
		create_collection_item(Ret++XParams),
		<<"collection-create-item">>;
	{<<"/item/bmcollection/update">>, _} ->
...
update_collection_item(Params) ->
	...
	{_, 1} ->
	    ...
	    CcR = rpc:call('edapi@127.0.0.1', model_bmcollection, update_item, [RID,Style,F,Color,Link,Sil,Pr,Hl,N,TL,SL,WS,IsD,RFI,RBI,Desc,SCU]),
	    lager:debug("UpdateCollectionItem Ret: ~p", [CcR]);
	{Aid, _} when is_integer(Aid), Aid>0 ->
	    case rpc:call('edapi@127.0.0.1', model_bmcollection, is_designer_item, [Aid, RID]) of
		true ->
...
create_collection_item(Params) ->
	...
	{_, 1} ->
	    ...
	    CcR = rpc:call('edapi@127.0.0.1', model_bmcollection, add_item, [CID,Style,F,Color,Link,Sil,Pr,Hl,N,TL,SL,WS,FI,BI,Desc,SCU]),
	    lager:debug("CreateCollection Ret: ~p", [CcR]);
	{Aid, _} when is_integer(Aid), Aid>0 ->
	    case rpc:call('edapi@127.0.0.1', model_bmcollection, is_designer_collection, [Aid, CID]) of
		true ->
