...
handle(Req, State) ->
    ...
    {ok, Req4} = reply(jiffy:encode({[{<<"type">>, <<"bg_image">>}, {<<"status">>, <<"ok">>}, 
		{<<"image">>, 	Img}
	]}), Req2),   
...
save_upload(Params) ->
    ...
    case {AccountID, IsAdmin} of
        {Aid, _} when is_integer(Aid), Aid>0 ->
            lager:debug("SAVE bg upload :: ~p", [{AccountID, FI}]),
            CcR = rpc:call('edapi@127.0.0.1', model_service_user, save_bg_upload, [AccountID, FI]),
...
get_file_name(UploadFileName0, InputName) ->
    ...
    TempDirectory = muploader_utils:get_tmp_dir(mbdbg, InputName),
